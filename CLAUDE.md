...
```bash
python scripts/run_ABCDEOOH_experiment.py --out runs/demo --only-generate --num-gen-eps 100 --reward-mode none
```
...
```bash
python scripts/run_ABCDEOOH_experiment.py --out runs/demo --use-saved-random-dataset --dqn-epochs 5 --num-gen-eps 50 --reward-mode none
```

### REINFORCE
```bash
python scripts/run_ABCDEOOH_experiment.py --out runs/reinforce --rl-method reinforce \
  --pg-warmup-eps 200 --pg-train-eps 1000 --num-gen-eps 200 --reward-mode none --pg-gen-stochastic
```
Use `--pg-gen-stochastic` with `--reward-mode none` to avoid duplicate compositions in generation (greedy argmax is deterministic with a uniform policy). With `--reward-mode dp` the policy learns real preferences and greedy generation works fine.
...
### Fair comparison across all three methods (DeepMD reward)
```bash
# DQN
python scripts/run_ABCDEOOH_experiment.py --out runs/dqn --rl-method dqn \
  --num-random-eps 1000 --dqn-epochs 50 --num-gen-eps 200 \
  --reward-mode dp --dp-poscar POSCAR --dp-model model_1.ckpt.pt --dp-model model_2.ckpt.pt
...
- With `--reward-mode none`, PG greedy generation always produces the same composition (deterministic policy + same initial state). Use `--pg-gen-stochastic` in this case. With `--reward-mode dp`, greedy argmax works well.
