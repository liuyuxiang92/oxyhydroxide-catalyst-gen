...
| `std_scaler.bin` | Serialized `StandardScaler` (joblib); fit once on initial random buffer |
...
- **`env.py`** — `ABCDEOOHEnv`: 5-step constrained environment. Fractions are internally tracked as integer units out of 20 (i.e., 0.05 = 1 unit). Uses `_possible_sums_by_k` to precompute feasibility so that every action guarantees a valid terminal state. The `terminal_formula` property canonicalizes cation order (major-first, then alphabetical). Each action is a pair of one-hot vectors `(elem_oh, comp_oh)`; `EpisodeStep` stores per-step state/action/reward.
- **`model.py`** — `QRegressor`: input = concat(scaled_material_features, step_onehot, elem_onehot, frac_onehot) → scalar Q value. Three linear layers with LeakyReLU; `hidden_dim=128` by default.
